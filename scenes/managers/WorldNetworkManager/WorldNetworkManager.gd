...
func _ready():
	...
	_signal = local_player_body_data.connect("deaths_changed", self, "_on_player_body_deaths_changed")
...
func _on_player_body_deaths_changed():
	_send_death_sync_packet(world_data.local_player_body_data)
...
func _send_death_sync_packet(player_body_data: PlayerBodyData):
	for member_id in get_lobby_member_ids():
		var player_body_death_sync_packet = PlayerBodyDeathSyncPacket.new()
		player_body_death_sync_packet.player_id = player_body_data.body.player_id
		_send_reliable_packet(member_id, player_body_death_sync_packet) 
